...
Luego de entrenado el modelo de red neuronal se pudieron recolectar los datos de exactitud arrojados de forma computacional en este caso se proyectó la recolección para un diseño experimental factorial de 〖2^k=2〗^3 con 10 repeticiones lo que arrojó 80 datos, luego se aplicaron las respectivas pruebas de normalidad, homocedasticidad e independencia, logrando obtener unos datos que se comportaron de manera normal validados con las pruebas de Shapiro-Wilk y Anderson-Darling, para el caso de la homocedasticidad se utilizó la prueba de Levane obteniendo unos valores por debajo de 0,05, lo que no permitió rechazar la hipótesis nula de la homogeneidad de las varianzas posiblemente asociado a los descensos de gradiente que este utiliza mediante el entrenamiento de la red, lo cual que miden la precisión utilizando iteraciones en las actualizaciones de los parámetros, hasta que esta logra estar lo más cercano a cero, por lo cual estos valores podrían ser no muy similares en cada una de las épocas utilizadas por las neuronas en el entrenamiento, lo mismo sucedió con el supuesto de independencia obtenido con la prueba de Durbin-Watson, los valores estuvieron por debajo de 0,05 la cual no permitió rechazar nuevamente la hipótesis nula del cumplimiento de independencia, probablemente asociado de igual modo a los descensos gradientes, que pudieron haber evitado que los valores de exactitud logrado por el modelo pudieran depender unos de otros, por la simple razón que al utilizar las épocas estos se iban ajustando desde el inicio hasta el final, donde cada nuevo aprendizaje dependía del anterior. Luego de realizado todos los supuestos se procedió a realizar los análisis experimentales teniendo como resultado valores de significancia para los factores de manera individual mas no para ninguna de sus interacciones, evidenciando que el modelo de manera independientes pudo haber capturado los efectos principales de manera más efectiva, por lo que los efectos fueron evaluados de forma aislada sin considerar interacciones complejas, ya que cada factor tenía un impacto claro en la respuesta del sistema, con esto podemos hacer ajustes individuales que puedan ayudar a maximizar la respuesta deseada en nuestro modelo ya que no es necesario tener en cuenta las complicaciones adicionales que son producto de las interacciones dentro del sistema.
